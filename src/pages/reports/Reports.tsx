...
const SalesTrendChart = ({
  ...
  if (!data || data.length === 0)
    return (
      <p className="text-sm text-text-secondary">No trend data available.</p>
    );

  const maxValue = Math.max(
    ...data.flatMap((d) => (showProfit ? [d.sales, d.profit] : [d.sales])),
    1,
  );
  const width = 800;
  const height = 250;
  ...
  const salesPath = data
    .map(
      (d, i) => `${i === 0 ? "M" : "L"} ${getX(i)} ${getY(d.sales, maxValue)}`,
    )
    .join(" ");
  const profitPath = showProfit
    ? data
        .map(
          (d, i) =>
            `${i === 0 ? "M" : "L"} ${getX(i)} ${getY(d.profit, maxValue)}`,
        )
        .join(" ")
    : "";
...
export const Reports = () => {
  ...
  const loadReports = async () => {
    ...
    try {
      const data = await reportApi.list(
        typeFilter ? { type: typeFilter as "WEEKLY" | "MONTHLY" } : undefined,
      );
  ...
  const exportExcel = async (report: Report) => {
      ...
      XLSX.utils.json_to_sheet([
        {
          ...
          Purchases: data.purchases.total,
          Expenses: data.expenses.total,
          NetProfit: data.netProfit,
          PartyOutstanding: balances.partyOutstanding,
          SupplierPayable: balances.supplierPayable,
        },
      ]),
      "Summary",
    );
    ...
    XLSX.writeFile(
      workbook,
      `${report.type.toLowerCase()}-report-${report.id}.xlsx`,
    );
  };
  ...
  const columns: DataTableColumn<Report>[] = [
    {
      ...
      cell: (row) => (
        <span
          className={`rounded-full px-2.5 py-1 text-xs font-semibold ${row.type === "WEEKLY" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"}`}
        >
          {row.type}
        </span>
      ),
      ...
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {isAdmin && (
            <Card className="p-4">
              ...
              <p
                className={`mt-1 text-xl font-bold ${data.netProfit >= 0 ? "text-green-700" : "text-red-700"}`}
              >
          ...
          <Card className="p-4">
            <p className="text-sm text-text-secondary">Party Outstanding</p>
            <p className="mt-1 text-xl font-bold text-secondary">
              {formatCurrency(balances.partyOutstanding)}
            </p>
            <p className="text-xs text-text-secondary">
              Amount receivable from parties
            </p>
          </Card>
          <Card className="p-4">
            <p className="text-sm text-text-secondary">Supplier Payable</p>
            <p className="mt-1 text-xl font-bold text-secondary">
              {formatCurrency(balances.supplierPayable)}
            </p>
            <p className="text-xs text-text-secondary">
        ...
        <Card className="p-6">
          <h3 className="mb-4 text-lg font-semibold text-secondary">
            {isAdmin ? "Sales & Profit Overview" : "Sales Overview"}
          </h3>
            ...
            {data.topProducts.length === 0 ? (
              ...
              <div className="space-y-3">
                {data.topProducts.map((product, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between rounded-lg border border-border-gold p-3"
                  >
                    <div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card className="p-6">
            ...
            {balances.parties.length === 0 ? (
              <p className="text-sm text-text-secondary">
                No outstanding party balances.
              </p>
            ) : (
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary">
              Generate and view weekly/monthly business reports
            </p>
          </div>
        </div>
        {canManage && (
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-secondary">
                ...
                {formatDate(selectedReport.periodStart)} to{" "}
                {formatDate(selectedReport.periodEnd)}
              </h3>
              <p className="text-sm text-text-secondary">
                Generated by {selectedReport.generatedBy?.name || "—"} on{" "}
                {formatDate(selectedReport.createdAt)}
              </p>
            </div>
            <div className="flex gap-2">
              {isAdmin && (
                <Button onClick={() => void exportExcel(selectedReport)}>
