...
export const CategoryMaster = () => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editing) {
        await productApi.categories.update(editing.id, form)
        toast({ title: 'Category updated', variant: 'success' })
      } else {
        await productApi.categories.create(form)
    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold text-secondary">Category Master</h2>
          </div>
        </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Edit Category' : 'Add Category'} footer={<><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" form="category-form">Save</Button></>}>
        <form id="category-form" onSubmit={submit} className="space-y-4">
          <FormField label="Category Name"><Input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></FormField>
