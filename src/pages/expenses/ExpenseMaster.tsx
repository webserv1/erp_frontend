...
const categories = ['petrol', 'collection', 'sample', 'bus', 'Transport', 'others']
const paymentModes = [
  { value: 'UPI', label: 'UPI' },
...
export const ExpenseMaster = () => {
  ...
  const [summary, setSummary] = useState<{ thisMonthTotal: number; totalRecords: number; activeExpenses: number } | null>(null)
  ...
  useEffect(() => {
    ...
    const loadExpenses = async () => {
      try {
        ...
        if (!cancelled) toast({ title: 'Failed to load expenses', description: (err as Error).message, variant: 'error' })
      } finally {
  ...
  }, [search, categoryFilter, paymentModeFilter, statusFilter, page, limit, toast])
  ...
  useEffect(() => {
    ...
    const loadSummary = async () => {
      try {
        ...
        if (!cancelled) toast({ title: 'Failed to load summary', variant: 'error' })
      }
    }
    loadSummary()
    return () => { cancelled = true }
    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-primary-dark">EXPENSE TRACKER</p>
            <h2 className="mt-1 text-2xl font-bold text-secondary">All Expenses</h2>
          </div>
        </div>
      ...
      <div className="mb-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <Card className="p-6">
          ...
          <p className="mt-1 text-2xl font-bold text-secondary">{loading ? '...' : `₹${(summary?.thisMonthTotal ?? 0).toLocaleString('en-IN')}`}</p>
        </Card>
        <Card className="p-6">
          <p className="text-sm text-text-secondary">Total Records</p>
          <p className="mt-1 text-2xl font-bold text-secondary">{loading ? '...' : (summary?.totalRecords ?? 0)}</p>
        </Card>
        <Card className="p-6">
          <p className="text-sm text-text-secondary">Active Expenses</p>
          <p className="mt-1 text-2xl font-bold text-secondary">{loading ? '...' : (summary?.activeExpenses ?? 0)}</p>
        </Card>
      </div>
        ...
        <form id="expense-form" onSubmit={submit} className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <FormField label="Details" required className="sm:col-span-2">
              <Textarea required value={form.details} onChange={(e) => setForm({ ...form, details: e.target.value })} placeholder="Enter expense details" />
            </FormField>
            <FormField label="Amount" required>
              <Input required type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="0.00" />
            </FormField>
            <FormField label="Payment Mode" required>
              <Select required value={form.paymentMode} onChange={(e) => setForm({ ...form, paymentMode: e.target.value as PaymentMode })}>
                  ...
                  {billPreviewUrl ? (
                    <div className="relative inline-block">
                      {form.billFile?.type.startsWith('image/') ? (
                        <img src={billPreviewUrl} alt="Bill preview" className="max-h-40 rounded border border-border-gold" />
                      ) : (
                        <div className="flex h-40 w-40 items-center justify-center rounded border border-border-gold bg-gray-50">
                          <span className="text-xs text-text-secondary">PDF Preview</span>
                        </div>
                      )}
                      <button
                        type="button"
                    ...
                    <a href={getBillUrl(form.billUrl)} target="_blank" rel="noreferrer" className="text-xs text-primary-dark underline">View current bill</a>
                  ) : null}
                </div>
              )}
            </FormField>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" loading={loading}>{editing ? 'Update Expense' : 'Submit Expense'}</Button>
          </div>
        </form>
      </Card>
      ...
      <Card className="mb-6 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <Select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setPage(1) }} className="sm:max-w-40">
              <option value="">All Categories</option>
              {categories.map((cat) => <option key={cat} value={cat}>{cat}</option>)}
            </Select>
            <Select value={paymentModeFilter} onChange={(e) => { setPaymentModeFilter(e.target.value); setPage(1) }} className="sm:max-w-40">
      ...
      <Modal open={!!viewing} onClose={() => setViewing(null)} title="Expense Details" footer={<Button variant="outline" onClick={() => setViewing(null)}>Close</Button>}>
        {viewing && (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <tbody className="divide-y divide-border-gold">
                <tr><td className="px-4 py-2 font-semibold text-text-secondary">Category</td><td className="px-4 py-2 text-secondary">{viewing.category}</td></tr>
                <tr><td className="px-4 py-2 font-semibold text-text-secondary">Details</td><td className="px-4 py-2 text-secondary">{viewing.details}</td></tr>
                <tr><td className="px-4 py-2 font-semibold text-text-secondary">Amount</td><td className="px-4 py-2 text-secondary">₹{viewing.amount}</td></tr>
                <tr><td className="px-4 py-2 font-semibold text-text-secondary">Payment Mode</td><td className="px-4 py-2 text-secondary">{viewing.paymentMode}</td></tr>
                {viewing.billUrl && (
                  <tr>
                    <td className="px-4 py-2 font-semibold text-text-secondary">Bill</td>
                    <td className="px-4 py-2 text-secondary">
                      <a href={getBillUrl(viewing.billUrl)} target="_blank" rel="noreferrer" className="text-primary-dark underline">View Bill</a>
                    </td>
                  </tr>
                )}
