...
export const Modal = ({ open, onClose, title, children, footer }: ModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-xl border border-border-gold bg-card shadow-xl">
        ...
        {footer && <div className="flex justify-end gap-3 border-t border-border-gold px-5 py-4">{footer}</div>}
      </div>
    </div>
  )
}
